...
class JavaEncoder(torch.nn.Module):
    ...
    def forward(self, data, batch):
        ...
        h = out.unsqueeze(0)
        edge_types = data.edge_index1[2]
        A1 = data.edge_index1[:2, edge_types == 0]

        feat_map = []
        for i in range(3):
            m = F.relu(self.conv(out, A1.long()))
...
class PyEncoder(torch.nn.Module):
    ...
    def forward(self, data, batch):
        ...
        feat_map = []
        edge_types = data.edge_index2[2]
        A1 = data.edge_index2[:2, edge_types == 0]
        for i in range(3):
            m = F.relu(self.conv(out, A1.long()))
            out, h = self.gru(m.unsqueeze(0), h)
            out = out.squeeze(0)
...
class Net(torch.nn.Module):
    def __init__(self, num_features, dim, use_unsup_loss=False, separate_encoder=False):
        ...
        # java encoder
        self.encoder1 = JavaEncoder(num_features, dim)

        # python encoder
        self.encoder2 = PyEncoder(num_features, dim)
        ...
        if separate_encoder:
            self.unsup_encoder1 = JavaEncoder(num_features, dim)
            self.ff11 = FF(2 * dim, dim)
            self.ff12 = FF(2 * dim, dim)
            self.unsup_encoder2 = PyEncoder(num_features, dim)
    ...
    def unsup_loss1(self, data, batch):
        ...
        if self.local:
            edge_types = data.edge_index1[2]
            A1 = data.edge_index1[:2, edge_types == 0]
            loss = local_global_loss_(l_enc, g_enc, A1.long(), batch, measure)
        return loss
    ...
    def unsup_sup_loss1(self, data, batch):
        ...
        edge_types = data.edge_index1[2]
        A1 = data.edge_index1[:2, edge_types == 0]
        loss = global_global_loss_(g_enc, g_enc1, A1.long(), batch, measure)
